...
2. 非空示例使用 `JSON.parse` 进行严格 JSON 语法校验，但不使用解析结果重新序列化；解析失败时保留成功、失败示例的全部原文，显示具体字段错误并停止保存。
3. 两个非空示例全部通过语法校验后，使用 `jsonc-parser` 基于原始词法标记生成两空格缩进的格式化文本。格式化过程不得把数字转换为 JavaScript `Number` 后再输出，必须保持大整数、指数形式和负零等数字词法值不变。
4. 成功、失败示例作为同一批次统一回写当前编辑模型，让管理员看到实际提交内容；不得在后一个示例校验失败时只改写前一个示例。
5. 使用格式化后的示例重新执行单字段 65,535 UTF-8 字节限制。
6. 使用包含格式化示例的完整保存快照执行 1 MiB 聚合请求体限制。
7. 继续执行数量、普通文本、树结构、必填项和 `READY` 完整性校验。
8. 全部通过后才调用聚合保存接口。
...
现有两个“格式化”按钮继续保留，便于管理员在保存前主动检查示例。手动格式化和保存动作调用同一个无损格式化工具，避免两条入口产生不同结果或任一入口损坏大整数。
...
- Windows 盘符绝对路径、Linux 明确系统路径，以及带内部文件系统上下文的 Linux 常见绝对路径。
...
- Windows 盘符路径继续直接拒绝。
- `/etc/passwd`、`/etc/shadow`、`/proc/*`、`/sys/*` 等明确系统路径直接拒绝。
- `/data/*`、`/home/*`、`/var/*`、`/usr/*`、`/opt/*`、`/root/*` 等存在公开 API 路径歧义的文本，只有在同一段文本还包含“服务器路径”“部署目录”“配置文件”“日志目录”等明确内部文件系统上下文时才拒绝。
- 单独出现的 `/data/users`、`/home/profile` 等公开路径允许保存，也不得被发布检查的接口 `path`、`url` 等字段误判。
...
- `/data/users`、`/home/profile` 等普通公开路径能够通过内容校验和发布检查。
- 明确系统路径以及“服务器路径：/data/app”等带内部文件系统上下文的路径仍保存失败。
...
- 手动格式化和保存前格式化均保持超出 JavaScript 安全整数范围的大整数、指数形式和负零原文不变。
- 任一 JSON 非法时成功、失败示例均保留格式化前原值且不发送保存请求。
