...
将管理员后台工作台从前端静态/Mock 状态改为真实数据库驱动，覆盖概览统计、最近 8 小时运行趋势、重点关注接口和最近变更；前端不保留任何工作台业务 Mock 数据。
...
运行趋势使用当前自然小时及此前7个自然小时，共8个一小时桶。当前小时作为最后一组，查询仅统计截至请求时刻已经产生的数据，不读取该小时的未来数据；后端补齐无数据桶，返回成功率、调用量、错误率和平均响应时间四组点。每个数据点通过带时区的 ISO 8601 标准时间字段 `time` 表示桶起始整点，前端统一格式化为 `MM-DD HH:mm`。百分比保留1位小数，响应时间单位为毫秒；无调用时对应比率为0。
...
删除 `dashboardMock.ts`、Mock 降级逻辑及 E2E 工作台伪造响应。工作台服务直接请求真实接口；页面使用独立结果归并，成功区块正常展示，失败区块显示错误提示和刷新入口。趋势无数据时显示明确空状态，不渲染误导性的空白图表；横轴时间显示为 `MM-DD HH:mm`，悬停时显示完整时间。告警和变更时间由前端统一格式化为相对时间。其他业务既有的单元测试桩和 E2E Fixture 不在本次清理范围内。
...
部署仓库在 `main` 分支修改 `docker-compose.yml`，将后端独立的 `sql/interface_change_log.sql` 挂载为第 09 个 MySQL 初始化脚本。
